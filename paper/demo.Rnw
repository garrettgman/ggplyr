...
\maketitle
This document demonstrates the basic features of the \texttt{glyphmaps} package (name may change). \texttt{glyphmaps} was written to create plots like the ones below. Each plot is made with \texttt{glyphmaps}. They recreate Figures 8 and 10a from \citet{wickham2012glyph}. To run the examples in this document please download the \texttt{glyphmaps} package with the code below.
...
p <- ggplot(seasons) + 
  ...
  glyph(
    ...
    major = aes(I(grid(lon)),I(grid(lat))), glyph.by = grid_by(lon, lat))
, 
    ref = ref_box(aes(fill = avg)))
@
