...
\label{sec:args}
Each \texttt{glyphmaps} geom takes two new arguments, \Sexpr{glyphs} and \Sexpr{reference}. \Sexpr{glyphs} controls how the data will be divided into subsets to be paired with individual glyphs. \Sexpr{reference} lets the user draw reference features with the glyphs to facilitate comparisons.

\Sexpr{glyphs} takes the name of the variable(s) to be used to divide the data into the subsets that will correspond to different glyphs. The \Sexpr{glyphs} argument works just like a \Sexpr{.vars} argument in a \Sexpr{ddply} call. (In fact, the \Sexpr{glyphs} argument in a \texttt{glyphmaps} geom will be passed into a \Sexpr{ddply} call as the \Sexpr{.vars} argument.)
...
<<comparison-with-boxes>>=
@

\section{Generalized glyphs}
Users are not limited to the pre-packaged glyph geoms. Any single layer plot can be turned into a glyph using the \Sexpr{glyph} and \Sexpr{plyr} functions.

<<bar-chart-glyph>>=
@

<<point-overplotting-ex>>=
@
